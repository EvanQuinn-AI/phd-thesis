...
session_defaults = {
    ...
    'data_yaml_path': None,
    'yolo_classes': [],
    'num_classes': 0  # Dynamically set after loading data.yaml
}
    ...
    try:
        ...
        with open(yaml_path, 'r', encoding='utf-8', errors='replace') as file:
            ...
            if class_names:
                ...
                st.session_state['num_classes'] = len(class_names)
            else:
...
class ActionRecognitionTransformer(nn.Module):
    ...
    def __init__(
        self,
        input_size=7,       # replaced at init with st.session_state['num_classes']
        ...
        dropout=0.1,
        num_classes=7       # replaced at init with st.session_state['num_classes']
    ):
        super().__init__()
...
def prepare_transformer_inputs_from_csv(
    ...
    if num_classes <= 0:
        raise ValueError("No valid number of classes found. Please load data.yaml first.")
    ...
    # Build a multi-hot vector for each frame
    ...
    while i + sequence_length <= len(frame_vectors):
        ...
        label_chunks.append(seq_tensor)  # seq-to-seq multi-label
        i += stride
...
def train_transformer_model(
    ...
    num_classes = st.session_state.get('num_classes', 0)
    if num_classes <= 0:
        st.error("No valid number of classes in session_state['num_classes']. Cannot train transformer.")
        return None
    ...
    for epoch in range(num_epochs):
        ...
        if len(val_ds)>0:
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                torch.save(model_.state_dict(), "models/transformer_model_best.pth")
            else:
                patience_counter += 1
                if patience_counter >= patience:
                    st.warning("Early stopping triggered!")
                    try:
                        model_.load_state_dict(torch.load("models/transformer_model_best.pth"))
                    except:
                        pass
                    break
...
# We import non_max_suppression from yolov5 utils
...
def process_video(video_path, model):
    """
    Example function that references classes by ID:
    0 -> boxing-bag, 1 -> cross, 3 -> hook, 4 -> kick, 6 -> person
    If your dataset doesn't have these classes, these references will be skipped if
    class_id >= len(class_names) or doesn't match the specific IDs.
    """
    ...
    # Last counted frame for each strike action
    last_hit_frame = {
        'cross': -1,
        'hook': -1,
        'kick': -1
    ...
    # Grace periods (in frames) per action
    action_grace_period = {
        'cross': 6,
        'hook': 8,
        'kick': 15
    ...
    while cap.isOpened():
        ...
        # Prepare lists for bounding boxes
        ...
        for det in detections:
            ...
            if confidence < CONFIDENCE_THRESHOLD:
                continue
            # Skip if class_id >= number of loaded classes
            if class_id >= len(class_names):
                continue

            # Check for known classes if dataset includes them
            # (the code won't break if they're missing; it just won't increment counters)
            if class_id == 0:  # 'boxing-bag'
                color = (0, 255, 0)
                class0_boxes.append((x1, y1, x2, y2))
            elif class_id == 1:  # 'cross'
                color = (255, 0, 255)
                class1_boxes.append((x1, y1, x2, y2))
            elif class_id == 3:  # 'hook'
                color = (255, 165, 0)
                class3_boxes.append((x1, y1, x2, y2))
            elif class_id == 4:  # 'kick'
                color = (0, 255, 255)
                class4_boxes.append((x1, y1, x2, y2))
            elif class_id == 6:  # 'person'
                color = (255, 0, 0)
                class6_boxes.append((x1, y1, x2, y2))
            else:
                # Some other class from your data.yaml
                color = (0, 0, 255)
        ...
        print(f"Frame {frame_count}: Detected Classes - "
              f"Cross: {len(class1_boxes)}, Hook: {len(class3_boxes)}, Kick: {len(class4_boxes)}, "
              f"Boxing Bags: {len(class0_boxes)}")
...
#########################
def load_transformer_model(model_path, d_model=64, nhead=2, num_layers=2, dim_feedforward=128, dropout=0.1):
    """
    Input/Output size is automatically derived from st.session_state['num_classes'].
    """
    num_classes = st.session_state.get('num_classes', 0)
    if num_classes <= 0:
        st.error("No valid number of classes in session_state['num_classes']. Please load data.yaml first.")
        return None
...
def model_execution():
    ...
    if st.button("Run Models") and video_file:
        ...
        df_detections = pd.DataFrame(
            [det for frame in all_detections for det in frame],
    ...
    if 'csv_file_path' in st.session_state:
        ...
        if st.button("Train Transformer"):
            try:
                seq_len=20
                stride=10
                inputs, labels = prepare_transformer_inputs_from_csv(df_, sequence_length=seq_len, stride=stride)
...
def generate_gpt_analysis(actions_sequence: List[str], stats_summary: Dict) -> str:
    ...
    if person_idx < len(durations) and bag_idx < len(durations):
        ...
        if person_presence > 90 and bag_presence > 90:
            ...
            insights.append("The session mainly involves sparring activities, showcasing interactive combat practice.")
...
def run_transformer_statistics(model_path, df_detections, video_path=None):
    ...
    num_classes = st.session_state.get('num_classes', 0)
    if num_classes <= 0:
        st.error("No valid number of classes. Please load data.yaml first.")
        return
    ...
    seq_len = 20
    stride  = 10
    inputs, _ = prepare_transformer_inputs_from_csv(df_detections, sequence_length=seq_len, stride=stride)
    if inputs.shape[0] == 0:
    ...
    # Merge chunk predictions => global_preds
    global_preds = {}
    N = len(inputs)
    for i in range(N):
    ...
    # Flicker filtering + instance counting
    # Hard-coded map for certain class references
    class_map = {
        0: "boxing-bag",
        1: "cross",
        2: "high-guard",
    ...
    for f_idx in frames_sorted:
        ...
        num_person = current[6] if 6 < num_classes else 0
        num_bag    = current[0] if 0 < num_classes else 0
        if num_person > 1:
            sparring_frames += 1
        ...
        for c_idx in range(num_classes):
            ...
            if consecutive_streak[c_idx] >= MIN_CONSECUTIVE.get(c_idx, 1) and not instance_in_run[c_idx]:
    ...
    for f_idx in frames_sorted:
        current = global_preds[f_idx]
        if (1 < num_classes and current[1] == 1) or \
           (2 < num_classes and current[2] == 1) or \
           (3 < num_classes and current[3] == 1) or \
           (4 < num_classes and current[4] == 1):
    ...
    chart_data = pd.DataFrame({
        'Class': [class_map[i] if i in class_map else f"Class_{i}" for i in range(num_classes)],
    ...
    for i in range(num_classes):
        label_ = class_map[i] if i in class_map else f"Class_{i}"
        st.write(f"{label_}: {instance_counts[i]}")
    ...
    for i in range(num_classes):
        if instance_counts[i] > 0:
            if i in class_map:
                ...
                actions_sequence.append(f"Class_{i}")
