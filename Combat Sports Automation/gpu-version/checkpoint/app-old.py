...
def process_video(video_path, model):
    ...
    iou_active = {'punch': False, 'kick-knee': False}
    ...
    while cap.isOpened():
        ...
        for det in detections:
            ...
            # separate boxes
            bag_boxes    = [tuple(map(int, d[:4])) for d in detections if int(d[5]) == 0 and d[4] > 0.4]
            punch_boxes  = [tuple(map(int, d[:4])) for d in detections if int(d[5]) == 5 and d[4] > 0.4]
            kick_boxes   = [tuple(map(int, d[:4])) for d in detections if int(d[5]) == 2 and d[4] > 0.4]

            # compute max IoU per action
            max_iou = {}
            for action, boxes in [('punch', punch_boxes), ('kick-knee', kick_boxes)]:
                best = 0.0
                for a in boxes:
                    for b in bag_boxes:
        ...
        for action in ['punch', 'kick-knee']:
            above = max_iou[action] >= IOU_THRESHOLD
            if overlap_results[action] and above and not iou_active[action]:
...
def run_transformer_statistics(model_path, df_detections, video_path=None):
    ...
    # Merge segments of the same action separated by ≤gap_max frames of another action
    def merge_segments(df, gap_max=2):
        merged = []
        prev = None
        for idx, row in df.iterrows():
            if prev is None:
                prev = row.copy()
            elif row['start'] - prev['end'] <= gap_max:
    ...
    # Apply to your punch segments:
    st.write("### Action Counts")
    punch_segments_merged = merge_segments(segs[segs['action'] == 'punch'], gap_max=1)
    st.write(f"Punches: {len(punch_segments_merged)}")
    gpt_punch = len(punch_segments_merged)
    print(gpt_punch)

    # Apply to your kick segments:
    kick_segments_merged = merge_segments(segs[segs['action'] == 'kick-knee'], gap_max=7)
    st.write(f"Kicks: {len(kick_segments_merged)}")
    gpt_kick = len(kick_segments_merged)
    print(gpt_kick)
    ...
    st.write("### Segment Length Distributions")
    ...
    system_prompt = """
    You are an expert combat-sports analyst.
    Produce exactly five observations, each:
    • In one sentence
    • Simple but accurate
    ...
    # Late Outputs
    st.write("### Markov Model Breakdown")
