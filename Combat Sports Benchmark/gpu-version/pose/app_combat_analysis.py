...
def _is_yolov5_model(model) -> bool:
    """Check if a model loaded by Ultralytics is actually a YOLOv5 architecture."""
    try:
        inner = model.model if hasattr(model, 'model') else model
        module_name = type(inner).__module__ or ''
        class_name = type(inner).__name__ or ''
        # YOLOv5 models expose 'models.yolo' or 'models.common' modules
        if 'yolov5' in module_name.lower() or 'models.yolo' in module_name.lower():
            return True
        # Check if fuse() accepts 'verbose' — YOLOv5 does not
        import inspect
        if hasattr(inner, 'fuse'):
            sig = inspect.signature(inner.fuse)
...
def load_yolo_model(model_path: str):
    """Load YOLO model — detects YOLOv5 vs v8+ and routes to the correct backend."""
    try:
        from ultralytics import YOLO
        model = YOLO(model_path)

        # Ultralytics can silently load YOLOv5 .pt files but they crash
        # during inference when fuse(verbose=...) is called. Detect and redirect.
        if _is_yolov5_model(model):
            st.warning("⚠️ Detected YOLOv5 architecture — reloading with torch.hub...")
            model = torch.hub.load('ultralytics/yolov5', 'custom',
                                   path=model_path, force_reload=False)
            st.info("✅ Loaded YOLOv5 model via torch.hub")
            return model, 'v5'
