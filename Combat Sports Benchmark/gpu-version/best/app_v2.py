...
def process_video(video_path, model):
    ...
    min_event_dur  = {'punch': 3,     'kick-knee': 6}
    gap_counter    = {'punch': 0,     'kick-knee': 0}
    gap_tolerance  = {'punch': 2,     'kick-knee': 4}
...
def run_transformer_statistics(model_path, df_detections, video_path=None):
    ...
    preds = (probs > 0.8).astype(int)  # threshold

    # Remove Duplicates
    df_sorted = df_detections.sort_values("confidence", ascending=False)
    df_unique = df_sorted.drop_duplicates(subset=["frame", "class_id"], keep="first")
    frames = df_unique['frame'].sort_values().values

    csv_path = video_path

    # Remove the prefix and change the extension
    video_stem = os.path.splitext(csv_path)[0].replace("yolo_predictions_", "")
    video_path = "runs/"+ video_stem
    ...
    # FPS & Path for Future Functionality
   ...
   # ----- use the *exact* thresholds from process_video -----
    ...
    #  Re-use from process_video function
    def check_overlap_(action_boxes, bag_boxes, frame_):
        for a in action_boxes:
            for b in bag_boxes:
                if boxes_intersect(a, b):
    ...
    # -----------------------------------------------------------------------------
    # 1) First, from df_unique compute the raw per-frame hits exactly as in process_video
    # -----------------------------------------------------------------------------
    raw_punch_frames = set()
    raw_kick_frames  = set()

    for f, grp in df_unique.groupby('frame'):
        bag_boxes   = [tuple(x) for x in grp[grp['class_id']==0][['x1','y1','x2','y2']].values]
        punch_boxes = [tuple(x) for x in grp[grp['class_id']==5][['x1','y1','x2','y2']].values]
        kick_boxes  = [tuple(x) for x in grp[grp['class_id']==2][['x1','y1','x2','y2']].values]

        # same geometry check
        if any(boxes_intersect(pb, bb) for pb in punch_boxes for bb in bag_boxes):
            raw_punch_frames.add(f)
        if any(boxes_intersect(kb, bb) for kb in kick_boxes  for bb in bag_boxes):
            raw_kick_frames.add(f)

    # -----------------------------------------------------------------------------
    # 2) Now *inline* the exact process_video FSM to count events
    # -----------------------------------------------------------------------------
    min_event_dur = {'punch': 3, 'kick-knee': 6}
    gap_tolerance = {'punch': 2, 'kick-knee': 4}
    ...
    for f in range(total_frames):
        for action, hits in [('punch', raw_punch_frames), ('kick-knee', raw_kick_frames)]:
            if f in hits:
                gap_counter[action] = 0
                if not in_event[action]:
                    in_event[action]   = True
                    event_start[action] = f
            else:
                if in_event[action]:
                    gap_counter[action] += 1
                    if gap_counter[action] >= gap_tolerance[action]:
                        # close event exactly as online
                        dur = f - event_start[action]
                        if dur >= min_event_dur[action]:
                            counters[action] += 1
                        in_event[action]    = False
                        gap_counter[action] = 0

    # final close any open event at end‐of‐video
    for action in ('punch','kick-knee'):
        if in_event[action]:
            dur = total_frames - event_start[action]
            if dur >= min_event_dur[action]:
                counters[action] += 1

    # -----------------------------------------------------------------------------
    # 3) Use those counters directly (and if you still need per‐segment DataFrames you can
    #    record segments inside that loop as well)… but for a brute‐force fix, this is all you need.
    # -----------------------------------------------------------------------------
    st.write("### Action Counts")
    # st.write(f"Punches: {counters['punch']}")
    # st.write(f"Kicks:   {counters['kick-knee']}")

    # Prepare for GPT
    gpt_punch = len(punch_segments)
    gpt_kick = len(kick_segments)
    st.write(f"Punches: {gpt_punch}")
    st.write(f"Kicks: {gpt_kick}")

   # gpt_punch = counters['punch']
   # gpt_kick = counters['kick-knee']
    ...
    # build a true instance‐count per action
    instances = {}

    # use your filtered segments for punch & kick
    instances['punch']     = len(punch_segments)
    instances['kick-knee'] = len(kick_segments)

    # for all other actions, just count segments in segs
    ...
    # Streamlit will render the styled DataFrame
    st.write(styled)
