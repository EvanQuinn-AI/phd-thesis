...
def run_transformer_analysis(model_path, csv_path, use_legacy,
    ...
    st.write("---")
    if st.checkbox("Generate LLM Analysis"):
        try:
            ...
            user_stats = {
                "Transformer_Punches": transformer_punch_count,
                "Transformer_Kicks": transformer_kick_count,
                "YOLO_Punches": yolo_punch_count,
                "YOLO_Kicks": yolo_kick_count,
                "Punch_F1": f"{punch_f1:.3f}" if not use_legacy else "N/A",
                "Kick_F1": f"{kick_f1:.3f}" if not use_legacy else "N/A"
            }
